...
3. Ingest in lock order after gold: Unihan readings, variants (t2s), kTGH (8105), CC-CEDICT, THUOCL, official luna, essay, emoji, zhwiki titles / page / category, then Tencent light vocab (`tencent-light`, coverage only). Luna/essay/emoji/tencent/wiki surfaces are simplified before overlay. Wiki-only and Tencent-only lemmas emit to bulk; Tencent overlay must not promote them into default `base` or `ext`. The Tencent pin is the ModelScope light subset, not the official ~8M dump.
