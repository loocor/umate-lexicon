...
8105, zhwiki titles, and authored brand/event packs are in. Wikipedia page/category dumps now reject wiki-only redirects and type inventory; wiki-only stays bulk. Tencent is pinned as the light ModelScope/Hugging Face Word2Vec vocabulary (coverage only; the official 6GB dump URL still returns HTML). Fetch + fusion/clean evidence is in [docs/tencent-validation.md](docs/tencent-validation.md); inventory lemma totals were not refreshed. Do not copy rime-ice `tencent.dict.yaml`.
