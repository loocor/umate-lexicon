...
字级繁简折叠、8105、维基标题、拉丁品牌金标已经进 locked pipeline。维基补了同日 `page`/`categorylinks` 做重定向排重和类型记账。有类型的 wiki-only 进对应 pack，冷权重，不进热表；`page_len` 只是篇幅分层，不是口语词频。纯 wiki / 纯 CC-CEDICT 不再往热表抬，也不在法律结论出来前剥离。见 [docs/zh/next-plan.md](docs/zh/next-plan.md)。覆盖盘点见 [docs/zh/inventory.md](docs/zh/inventory.md)。CI 继续走 fixture；本地 `fetch` 会拉约 111MB 的 light `.bin` 和约 118MB 的 d200 GBK key 表。
